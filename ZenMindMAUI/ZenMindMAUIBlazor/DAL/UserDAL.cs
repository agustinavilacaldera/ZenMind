  ...
  public class UserDAL
  {
    ...
    public ResultModel<Boolean> CheckLogin(MUser user)
    {
      ...
      try
      {
        ...
        List<MUser> Users = (from x in con.Table<MUser>() where x.Email.Equals(user.Email) && x.Password.Equals(user.Password) select x).ToList();
    ...
    public ResultModel<bool> GetUsers()
    {
      ...
      {
        SQLiteConnection con = SqlConnection.GetConnection();
        List<MUser> Users = (from x in con.Table<MUser>()
                     select x).ToList();
